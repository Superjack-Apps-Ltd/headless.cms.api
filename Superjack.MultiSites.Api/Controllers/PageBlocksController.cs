...
namespace Superjack.MultiSites.Api.Controllers
{
  ...
  public class PageBlocksController : ControllerBase
  {
    ...
    [HttpGet]
    [Route("~/pageblocks/pageid/{pageid}/{parentid}/{level}")]
    public IActionResult GetAllByPageIdParentIdLevel(long pageid, long parentid, int level)
    {
      var itemDtos = GetPageBlocks(pageid, parentid, level);
      return Ok(itemDtos);
    }
